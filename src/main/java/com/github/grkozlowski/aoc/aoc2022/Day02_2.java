...
public class Day02_2 {
    ...
    private static final Map<Character, Map<Character, Character>> remapper = Map.ofEntries(
            entry('X', Map.ofEntries(
                    entry('A', 'Z'),
                    entry('B', 'X'),
                    entry('C', 'Y')
            )),
            entry('Y', Map.ofEntries(
                    entry('A', 'X'),
                    entry('B', 'Y'),
                    entry('C', 'Z')
            )),
            entry('Z', Map.ofEntries(
                    entry('A', 'Y'),
                    entry('B', 'Z'),
                    entry('C', 'X')
            ))
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (String line : input) {
                ...
                myMove = remapper.get(myMove).get(opponentMove);
